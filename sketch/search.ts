...
function setupSearch(datas: p5.Table) {
    for (var i = 0; i < datas.getRowCount(); i++) {
        var author = datas.getRow(i).get("AuthorNames-Deduped").toString();
        if (authors.indexOf(author.trim()) < 0) {
            authors.push(author.trim());
        }
    }
    autocomplete(<HTMLFormElement>document.getElementById("search"), authors);
}
...
function autocomplete(inp: HTMLFormElement, arr: string[]) {
    ...
    inp.addEventListener("keydown", function (e) {
        ...
        if (x) x = x.getElementsByTagName("div")[0];
        if (e.keyCode == 40) {
